...
class Music(commands.Cog):
    def __init__(self, bot):
        self.bot = bot
        self.votes = []
        self.bot.embed_color = 0xF15A24

        if not hasattr(bot, 'lavalink'):  # This ensures the client isn't overwritten during cog reloads.
            bot.lavalink = lavalink.Client(760415780176658442)
            bot.lavalink.add_node('lavalink.something.host', 80, 'youshallnotpass', 'eu')  # Host, Port, Password, Region, Name
    ...
    async def ensure_voice(self, ctx):
        ...
        if not ctx.author.voice or not ctx.author.voice.channel:
            embed = discord.Embed(
                ...
                description="• Please join any voice channel to listen to music!"
            )
        ...
        if not player.is_connected:
            ...
            if not permissions.connect or not permissions.speak:  # Check user limit too?
                embed = discord.Embed(
                    ...
                    description="• Please give me connect permissions, or speaking permissions in the server!"
                )
            ...
            if int(player.channel_id) != ctx.author.voice.channel.id:
                embed = discord.Embed(
                    color=self.bot.embed_color,
                    title=" Voice Channel Error!",
                    description="• Please make sure we both are in same voice channel!"
                )
                raise commands.CommandInvokeError(await ctx.send(embed=embed))
    ...
    async def play_error(self, ctx, error):
        if isinstance(error, commands.MissingRequiredArgument):
            embed = discord.Embed(
                ...
                description="• Please put a valid option! Example: `!play <Song Name / URL>`"
            )
            await ctx.send(embed=embed)
    ...
    async def seek_error(self, ctx, error):
        if isinstance(error, commands.MissingRequiredArgument):
            embed = discord.Embed(
                color=self.bot.embed_color,
                title=" Invalid Argument!",
                description="• Please put a valid option! Example: `!seek <time>`"
            )
            await ctx.send(embed=embed)
    ...
    async def remove_error(self, ctx, error):
        if isinstance(error, commands.MissingRequiredArgument):
            embed = discord.Embed(
                color=self.bot.embed_color,
                title=" Invalid Argument!",
                description="• Please put a valid option! Example: `!remove 1`"
            )
            await ctx.send(embed=embed)

    @commands.command()
    async def fin(self, ctx, *, query):
    ...
    async def find_error(self, ctx, error):
        if isinstance(error, commands.MissingRequiredArgument):
            embed = discord.Embed(
                color=self.bot.embed_color,
                title=" Invalid Argument!",
                description="• Please put a valid option! Example: `!search <song>`"
            )
            await ctx.send(embed=embed)
